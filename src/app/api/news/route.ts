  ...
  try {
    ...
    return feed.items.slice(0, 10).map((item, index) => {
      ...
      return {
        ...
        content,
        cover: item.enclosure?.url || `https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=400&h=300&fit=crop&q=80`,
      ...
      try {
        ...
        if (title && title.length > 10) { // Filter out navigation items
          ...
          articles.push({
            ...
            shortDescription: content.slice(0, 200) + (content.length > 200 ? '...' : ''),
            content,
            cover: image.startsWith('http') ? image : (image.startsWith('/') ? `${new URL(source.url).origin}${image}` : `https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=400&h=300&fit=crop&q=80`),
